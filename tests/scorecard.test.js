...
const answersForTotal = (total) => {
  ...
  let remaining = total;
  for (const categoryId of categoryIds) for (let index = 0; index < 4; index += 1) {
...
test('calculation preserves every required boundary total', () => {
  [0, 20, 21, 40, 41, 60, 61, 80].forEach((total) => assert.equal(calculateScorecard(answersForTotal(total)).total, total));
});

test('server accepts exactly twenty valid answers', () => {
  const normalized = normalizeAnswers(answersWithValue(3));
  assert.ok(normalized);
  assert.equal(Object.keys(normalized).length, 20);
  assert.equal(questionKeys.length, 20);
});

test('server rejects missing, extra and out-of-range answers', () => {
  const missing = answersWithValue(2); delete missing['artistIdentity-0'];
  const extra = { ...answersWithValue(2), unexpected: 2 };
  const invalid = { ...answersWithValue(2), 'artistIdentity-0': 5 };
  assert.equal(normalizeAnswers(missing), null);
  assert.equal(normalizeAnswers(extra), null);
  assert.equal(normalizeAnswers(invalid), null);
});

test('server recalculates result from raw answers', () => {
  const answers = normalizeAnswers(answersForTotal(61));
